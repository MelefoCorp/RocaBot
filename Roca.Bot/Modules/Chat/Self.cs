...
namespace Roca.Bot.Modules.Chat
{
    ...
    public class Self : RocaBase
    {
        ...
        public class Test : RocaBase
        {
            ...
            {
                var msg = await Context.RespondAsync(GetType().GetLocalizer()["ping"]).ConfigureAwait(false);
                await Task.Delay(1000).ConfigureAwait(true);
                var account = await Context.User.GetAccount(false).ConfigureAwait(false);
                await msg.ModifyAsync(x => x.Content = GetType().GetLocalizer()["pong"].Value).ConfigureAwait(false);
